...
parser.add_argument('--output_path', type=str,default='pretrained', help='output path')
...
a2b_model_path = "./checkpoint/pretrained/{}_a2b.pth".format(opt.dataset)
b2a_model_path = "./checkpoint/pretrained/{}_b2a.pth".format(opt.dataset)
...
if not os.path.exists(os.path.join('./result', opt.output_path, opt.dataset)):
    os.mkdir(os.path.join('./result', opt.output_path, opt.dataset))
    os.mkdir(os.path.join('./result', opt.output_path, opt.dataset, 'a2b'))
    os.mkdir(os.path.join('./result', opt.output_path, opt.dataset, 'b2a'))
...
for image_name in a_image_filenames:
    ...
    save_img(b_gen,Ha,Wa, "./result/{}/{}/{}/{}".format(opt.output_path, opt.dataset, 'a2b', image_name))
...
for image_name in b_image_filenames:
    ...
    save_img(a_gen,Hb,Wb, "./result/{}/{}/{}/{}".format(opt.output_path, opt.dataset , 'b2a', image_name))
